...
- `records`：二十五份档案，编号格式为 `X列号-两位列内序号`，如 `X1-01`、`X2-01`；五列分别为 `X1-01`～`X1-05` 至 `X5-01`～`X5-05`。每个分类必须恰好有五份，同分类在数组中的顺序决定列内顺序。
...
当前五列、每列五份与原片布局、循环阵列相连。校验会拒绝增减档案或改变分类数量；调整这些数量需要另行修改阵列逻辑。列号由 `columns` 顺序决定，列内序号由同分类在 `records` 中的顺序决定。修改排序时须同步更新编号和下载文件，保持列内顺序一致。校验可以检查编号格式与排列，无法判断改写后是否仍是原来那份档案。
