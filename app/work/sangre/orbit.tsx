...
const chapters = [
  ...
  { label: ["结构", "Structure"], title: ["从外在体验，到内部秩序。", "Considered, inside out."], intro: ["分层展开主要组件，查看造型背后的装配关系。", "A layered view reveals the assembly behind the form."], left: ["显示组件与外壳", "Display & enclosure"], leftBody: ["保留原始装配几何，通过分层展示理解各部件的位置关系。", "Original assembly geometry is separated to make the relationship between parts legible."], right: ["从结构走向原型", "From assembly to prototype"], rightBody: ["下方保留工图与实物验证记录。电子检测功能属于概念方案。", "Drawings and physical validation follow below. Electronic testing remains conceptual."] },
] as const;
...
export default function SangreOrbit({ locale = "zh" }: { locale?: "zh" | "en" }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (next !== active.current) { active.current = next; setChapter(next); setFree(false); api.current?.chapter(next); }
    };
  ...
  return <section ref={section} className="sg-orbit" aria-label={t("SANGRE 三维产品讲解", "SANGRE 3D product story")}>
    <div className="sg-stage" data-chapter={chapter}>
      ...
      <div className={"sg-annotations " + (free ? "sg-annotations--hidden" : "")} key={"notes-" + chapter}>
